    ...
    <div class="card">
        <div class="card-body">
            ...
            @if ($errors->any())
                ...
                <br>
            @endif
            <form method="POST" action="{{ route('egreso.store') }}" enctype="multipart/form-data">
	            @csrf
	            <div class="row">
	            	<div class="col-xs-4 col-sm-12 col-md-4">
                        <div class="form-group">
                            <strong>Nro egreso:</strong>
                            <input type="text" name="egreso" readonly  value="{{$code}}" value="{{ old('egreso') }}" class="form-control" placeholder="IG000001">
                        </div>
                    </div>
                    <div class="col-xs-4 col-sm-12 col-md-4">
                        <div class="form-group">
	            ...
	            <div class="card">
	                ...
	                <div class="card-body">
	                    <div style="margin-bottom: 10px;" class="row">
	                        <div class="col-lg-12">
	                            <button type="button" class="btn btn-success add-egreso-detail">
	                                <i class="fa fa-plus" aria-hidden="true"></i> Agregar
	                            </button>
	                        </div>
	                    </div>
	                    <table class="table table-bordered table-striped table-hover egreso_detail">
	                        <thead>
	                            <tr>
	                                <th>Concepto</th>
	                                <th>Cantidad</th>
	                        ...
	                        <tbody>
	                        	@foreach(old('concept_id', ['']) as $key => $ingresoDetail)
	                                <tr>
	                                    ...
	                                    <td>
	                                        <button type="button" class="btn btn-xs btn-danger delete-egreso-detail" title="Quitar">
...
<script>
$(function () {
    let $options = $('table.egreso_detail tbody');
    let index = $options.find('tr').length;
    $('.add-egreso-detail').click(function (e) {
        e.preventDefault();

        if($options.find('tr:last select').val() == null)
        	alert("Por favor, seleccione el concepto de egreso");
    ...
    $options.on('click', '.delete-egreso-detail', function (e) {
        e.preventDefault();
